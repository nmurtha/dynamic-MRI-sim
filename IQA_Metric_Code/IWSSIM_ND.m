function [ score, scaleScores, infoWeights ] = IWSSIM_ND( imgRef, imgDis, numScales,...
    ssimWinDims, infoWindow, pyrWindow, maxGrayScale )

%  WRITTEN: Nathan Murtha (nathan.j.murtha@gmail.com)
%           Feb 21, 2017. Edited April 4, 2017.
%
%  PURPOSE: Extend the IW-SSIM to 3D images. 
%
...
%
%   numScales   -> The number of scales in the Laplacian pyramid that will
%                  be considered in calculating the IW-SSIM score. By
...
% 
%   infoWindow   -> A vector with sizes of window used in calculation of 
%                   info map. By default, taken as 3 in each dimension.
%
%   pyrWindow    -> Size of window to be used in making the
%                   Laplacian pyramid. A gaussian filter of size pyrWindow 
%                   by pyrWindow is made. By default, pyrWindow = 5.
%
...
% 
%  OUTPUTS:
%   score       -> Score for the IW-SSIM metric of the distorted image.
%
%   scaleScores -> Score at each scale in the Laplacian pyramid.
%
%   infoWeights -> Cell array of information weighting maps used for the
%                  calculation.
%
%  EXTERNAL FUNCTIONS:
%   getInfoMapsVolumeWeighting.m
%   getLaplacePyramid.m
...
if exist('numScales','var')
    assert(numScales <= 5,'Only support 5 pyramid scales (empirical weights).')
end

% ------------------------   SET DEFAULTS IF NEEDED   ---------------------
% Check pyramid window side length, set to 5 by default.
...
% Check if infoWindow is set, use default value of 3 in each dimension.
if (~exist('infoWindow','var'))
   if numel(dimsRef) == 3
      infoWindow = [3 3 3]; % 3D image input
   else
      infoWindow = [3 3 1]; % 2D image input
   end
end
...
% Set default stability parameters from Wang's SSIM paper.
K  = [0.01 0.03];
C1 = (K(1)*maxGrayScale)^2;
C2 = (K(2)*maxGrayScale)^2;
C3 = C2/2;

% Set pyramid scale weights from Wang's MS-SSIM paper (he assumes same 
% weighting scheme in IW-SSIM application, with equal weighting within 
% scales between luminance, contrast and structure). 
...
% Get Laplacian Pyramid of each image.
refPyramid = getLaplacePyramid(imgRef, numScales, pyrWindow); 
disPyramid = getLaplacePyramid(imgDis, numScales, pyrWindow);
    
% Calculate info weight maps. At the highest scale, no info weighting is 
% performed (see IW-SSIM paper) so map consists entirely of ones.
infoWeights = getInfoMapsVolumeWeighting(refPyramid,disPyramid,numScales,...
    infoWindow(1),infoWindow(2),infoWindow(3),0.4); 
...
for iScale = 1:numScales
    ...
    infoWeightTemp = infoWeights{iScale};
    scaleScores(iScale) = sum(sum(sum(tempScaleMap.*infoWeightTemp)))...
        ./sum(sum(sum(infoWeightTemp)));
end
