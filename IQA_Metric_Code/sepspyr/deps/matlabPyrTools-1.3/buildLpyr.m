...
%
% Construct a Laplacian pyramid on array IM.
%
...
% Eero Simoncelli, 6/96. Edited by Nathan Murtha, Feb 2017.

function [pyr,pind] = buildLpyr(im, height, filt1, filt2, edges)
...
% Assign default filters if none were assigned.
if (exist('filt1','var') ~= 1)
  filt1 = 'binom5';
end

if (exist('filt2','var') ~= 1)
  filt2 = filt1;
end

% If user specifed filters, look them up and assign from namedFilter.m.
if ischar(filt1)
...
% Check that filters are vectors and turn them into a row vector.
...
% Figure out number of scales, assigning default as maximum number if
% height of pyramid is unspecified.
max_ht = 1 + maxPyrHt(im_sz, max(size(filt1,1), size(filt2,1)));
if ( (exist('height','var') ~= 1) | (height == 'auto') )
  height = max_ht;
else
  if (height > max_ht)
...
if (height <= 1)
  ...
  [npyr,nind] = buildLpyr(lo2, height-1, filt1, filt2, edges);
