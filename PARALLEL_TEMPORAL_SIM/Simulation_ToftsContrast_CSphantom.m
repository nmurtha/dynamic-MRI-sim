...
% below).
TR              = 0.005;  % [sec], one TR between frequency encodes (i.e. points in CIRCUS phase tables).
dTimePerfect    = 1;     % [sec], time lag between snapshot sampled patterns (only used if TR = 0).
numKspaceAcq    = 200;     % Pairs of calib/quanta sampling patterns to acquire (e.g. numKspaceAcq = 2 samples 2 pairs of CQ)

numCoils        = 1;     % Number of coils to use in parallel imaging.

numCIRCUSPregen = 150;     % Number of CIRCUS patterns to pregenerate (will be looped through if needed).
CIRCUSmode      = 1;     % =1 specifies # quanta to return, =2 specifies undersampling factor.
CIRCUSreturn    = 5;     % quanta per pattern for mode=1, undersampling factor for mode=2.
...
numCalInterleave    = 1;      % number of calibration regions to interleave.
numQuantaInterleave = 5;      % number of quanta to interleave.
numCalIncrement     = 1;      % jump between calibration regions when interleaving.
patternTimePriority = 'calib';% 'calib' to set effective times based on calibration regions, 'equal' to include quanta with calibration regions 
...
BARTregularization = '-R W:7:0:0.02'; 
...
% Get the phantom.
[ phantBase, phantFeatures ] = Get_CS_Phantom_Tofts( phantNumRows, phantNumCols,... % in-slice phantom size
    samplingTimes); % vector of times at which phantom sampling occurs
...
paramSlice4_BART = Get_Tofts_Parameters( BARTrecons, 4,...
