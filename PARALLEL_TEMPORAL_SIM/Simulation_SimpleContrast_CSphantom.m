...
%  ------------------------------------------------------------------------
% 
%  User defined simulation parameters are set here. These incude the
%  phantom cross sectional size, addition of noise in image space, the 
%  CIRCUS parameters, the CS regularization, and so on.
%
...
% Set phantom properties. NOTE: DO NOT change phantNumSlices without also 
% manually changing the number of slices in Get_CS_Phantom.m.
phantNumRows    = 200;
phantNumCols    = 200;
phantNumSlices  = 32;  % FIXED in Get_CS_Phantom.m, DON'T CHANGE.
maxSignalValue  = 255; % Max range of signal obtained in phantom, in arbitrary units. Shouldn't really matter...
...
% below).
TR              = 0.00;  % [sec], one TR between frequency encodes (i.e. points in CIRCUS phase tables).
dTimePerfect    = 2;     % [sec], time lag between snapshot sampled patterns (only used if TR = 0).
numKspaceAcq    = 40;    % Pairs of calib/quanta sampling patterns to acquire (e.g. numKspaceAcq = 2 samples 2 pairs of CQ)

numCoils        = 2;     % Number of coils to use in parallel imaging.
...
BARTregularization = '-R W:7:0:0.01'; 
...
[ CIRCUScalib, CIRCUSquanta ] = Generate_CIRCUS_PhaseTables(...
    phantNumRows, phantNumSlices,... % frequency encodes will run "in-slice", so phase encodes depend on number of rows and slices
...
%                      i-th phantom feature in a 3D array.
...
%                   -> phantFeatures(i).params contains the true temporal
%                      parameters for the i-th phantom feature in a vector.                      
...
% Get the phantom.
[ phantBase, phantFeatures ] = Get_CS_Phantom_SimpleContrast( phantNumRows, phantNumCols,... % in-slice phantom size
    samplingTimes,... % vector of times at which phantom will be sampled
    maxSignalValue ); % max signal value obtained by phantom
...
% Get embedded cylinders mask, using first five features. NOTE: make sure
% that features 1 through 5 are actually of the embedded cylinder variety,
% and that the slice of the feature shapes being used to make the mask is
% correct. 
maskEmbedCyl = zeros(phantNumRows,phantNumCols);
for iFeat=1:5
    maskEmbedCyl = maskEmbedCyl + phantFeatures(iFeat).shape(:,:,4);
end

% Get pin grids mask. NOTE: make sure that the correct feature number is 
% being used, and that the appropriate slice is referenced in making the 
% mask.
maskPingrids = zeros(phantNumRows,phantNumCols);
maskPingrids = maskPingrids + phantFeatures(17).shape(:,:,28);
...
paramSlice4_BART = Get_Exponential_Parameters( BARTrecons, 4,...
...
paramSlice10_BART = Get_Exponential_Parameters( BARTrecons, 10,...
    effSampleTimes, maskEmbedCyl );
...
paramSlice16_BART = Get_Sinusoidal_Parameters( BARTrecons, 16,...
    effSampleTimes, maskEmbedCyl );
...
paramSlice16_True = zeros(phantNumRows,phantNumCols,4);
for i = 1:4
    for j = 11:15
